...
class Form(qtw.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.ylim_min_in_fd = qtw.QLineEdit("-0.1")
        self.ylim_max_in_fd = qtw.QLineEdit("1.1")
        self.ylim_min_in_td = qtw.QLineEdit("0")
        self.ylim_max_in_td = qtw.QLineEdit("1")
        self.xlim_min_in_fd = qtw.QLineEdit("-330")
        self.xlim_max_in_fd = qtw.QLineEdit("330")
        self.xlim_min_in_td = qtw.QLineEdit("0")
        self.xlim_max_in_td = qtw.QLineEdit("1.5")
        ...
        # slider
        self.fd_slider = qtw.QSlider(qtc.Qt.Horizontal)
        self.td_slider = qtw.QSlider(qtc.Qt.Horizontal)
        ...
        layout.addWidget(self.fd_slider, 9, 5, 1, 1)
        ...
        layout.addWidget(self.td_slider, 9, 13, 1, 1)
        ...
        self.axis_y.setMin(-0.1)
        self.axis_y.setMax(1.1)
        ...
        self.axis_x_td.setTitleText("Time (ns)")
        self.axis_x_td.setMin(0)
        self.axis_x_td.setMax(1.5)
    ...
    def confirm(self):
        self.plot_path, _ = qtw.QFileDialog.getOpenFileName(
            self, "Select File")
    ...
    def periodic_refresh(self):
        ...
        # a will be frequency domain, c will be time domain
        x_stream_c, y_stream_c, x_stream_a, y_stream_a = bw.eye(freq, Re, Im)
        # x_stream_a = s_freq_S21
        # y_stream_a = abs(S21)
        # x_stream_c = warped_ty
        # y_stream_c = mag_y
        x_stream_c = x_stream_c * 1e9  # because it's in nanoseconds

        # This is what eye() returns:
        # return warped_ty, mag_y, s_freq_S21, abs(S21)

        self.data_stream_series_a.clear()
        self.data_stream_series_c.clear()

        for i, xi in enumerate(x_stream):       # make
            self.data_stream_series_c.append(x_stream_c[i], y_stream_c[i])

        x_stream2 = np.linspace(-330, 330, 6002)
        for i, xi in enumerate(x_stream2):       # make
            self.data_stream_series_a.append(x_stream_a[i], y_stream_a[i])
...
if __name__ == "__main__":
    ...
    if style == 1:
        ...
        dark_palette.setColor(qtg.QPalette.WindowText, qtc.Qt.white)
        dark_palette.setColor(qtg.QPalette.Base, qtg.QColor(25, 25, 25))
        dark_palette.setColor(qtg.QPalette.AlternateBase,
                              qtg.QColor(53, 53, 53))
        dark_palette.setColor(qtg.QPalette.ToolTipBase, qtc.Qt.white)
        dark_palette.setColor(qtg.QPalette.ToolTipText, qtc.Qt.white)
        dark_palette.setColor(qtg.QPalette.Text, qtc.Qt.white)
