...
def eye(freq, Re, Im):
    ...
    return warped_ty, mag_y, s_freq_S21[2000:3000], abs(S21[2000:3000])

# You are going to have to play around with arrays more and concatenate them in different order if you want -330 to 330
...
# eye_diag_window = 3*tb  # arbitrary, we typically display 3 tb in eye diagram
# ax[0].scatter(np.mod(ty, eye_diag_window), mag_y, s=marker_size)  # eye diagram

# s_freq_S21 = np.fft.fftfreq(len(S21), d=(1/(2*max(freq))))
# ax[1].plot(s_freq_S21, abs(S21))    # S21 we are trying to plot
...
# FOLLOWING CODE WILL NOT BE INCLUDED IN PyPNA_Gui.py
# Reason being as long as our code works, we will put into it and...
# # COMMENT START
